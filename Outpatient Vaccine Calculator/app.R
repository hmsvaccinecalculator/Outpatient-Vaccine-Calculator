        ...
        column(width = 4,
         ...
         box(title = "Contact", width = NULL, solidHeader = TRUE, status = input_element_color,
             ...
             numericInput("contact-staffing", "Outreach and Scheduling Staffing (USD, per diem cost per worker per hour)", value = 27,
                          min = 0),
             numericInput("contact-screening", "Outreach and Pre-screening Time (Minutes per outreach, including callbacks)", value = 7,
                          min = 0),
...
server <- function(input, output) {
 ...
 ##Backend Variable Calculations------------------------------ 
  ...
  #Non-Wage Variables for 'Totals' Table
    vaxperday <- reactive({
      (activehours()*60/(input$`vaccination-id` + input$`vaccination-disposal` + 
  ...
  #Variables for 'Per Subphase' Staff Hours Table
    ...
    scheduling_persub <-reactive({
      ((input$`contact-screening`+ input$`contact-scheduling`)*input$`contact-outreaches`/60)+(input$`scheduling-probability`*input$`contact-outreaches`*input$`scheduling-cancel`/60)
    })
    ...
    vaxprep_persub <-reactive({
      input$`contact-outreaches`*(input$`equipment-supplyorder`/100+
          input$`backroom-needleprep`/24+input$`backroom-vialprep`/24)/60+
            input$`closingup-storage`*weekspersub()*input$`scheduling-availability`
    })
  
    vaxadmin_persub <-reactive({
      input$`contact-outreaches`*(input$`safety-donning`+input$`vaccination-id`+
        input$`vaccination-disposal`+input$`cleanup-doffing` +input$`documentation-reporting`)/
          60 + input$`contact-outreaches`*input$`observation-anaphylaxis`+input$`observation-vaers`
    })
  
    custodian_persub <-reactive({
      ((input$`waiting-setup`+input$`waiting-cleanup`)*2/60+input$`closingup-cleanup`)*weekspersub()*input$`scheduling-availability`
    })
  #Variables for 'Per Subphase' Staff Wages Table
    medwages_persub <-reactive({
      (scheduling_persub()*input$`contact-staffing`+registration_persub()*input$`intake-wages`+vaxprep_persub()*input$`backroom-wages`)*(1+input$`staffing-overhead`)
    })
  ...
  #Wage Variables for the 'Totals' Table
    personnel_pervax<-reactive({
      (13*(medwages_persub()+nursewages_persub()+custodialwages_persub()))/
        totalvax()
    })
    ...
    equipment_pervax<-reactive({
      (input$`equipment-ppecost`*weekspersub()*input$`scheduling-availability`+
         input$`equipment-supplyorder`/100*input$`contact-outreaches`+
          input$`observation-supplies`)*13/totalvax()
    })
  ...
  output$staffhours = renderDataTable({
    ...
    perweek1 = c(round(logistics_persub()/weekspersub()),
    ...
    persub1 = c(round(logistics_persub()),
                round(it_persub()),
                round(pharm_persub()),
    ...
    total1 = c(round(13*logistics_persub()+logistics_onetime()),
  ...
  output$staffwages = renderDataTable({
    ...
    perweek2 = dollar_format()(c(round(medwages_persub()/weekspersub()),
                 round(nursewages_persub()/weekspersub()), 
                 round(custodialwages_persub()/weekspersub()),
                 round((medwages_persub()+nursewages_persub()+custodialwages_persub())/weekspersub())
                 ))
    persub2 = dollar_format()(c(round(medwages_persub()),
                round(nursewages_persub()),
                round(custodialwages_persub()),
                round(medwages_persub()+nursewages_persub()+custodialwages_persub())
              ))
    total2 = dollar_format()(c(round(13*medwages_persub()),
               round(13*nursewages_persub()),
               round(13*custodialwages_persub()),
               round(13*(medwages_persub()+nursewages_persub()+custodialwages_persub()))
              )) 
  ...
  output$vaccination_cost_box2 <- renderValueBox({
    valueBox(dollar(13*(medwages_persub()+nursewages_persub()+custodialwages_persub())), "Total Per Diem Personnel Costs",
             icon = icon("dollar-sign"),
             color = "maroon")
  })
  
  output$vaccination_cost_box3 <- renderValueBox({
    valueBox(dollar((medwages_persub()+nursewages_persub()+custodialwages_persub())/weekspersub()), "Weekly Per Diem Personnel Costs",
             icon = icon("dollar-sign"),
             color = "aqua")
  })
}
